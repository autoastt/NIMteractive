      ...
      <div className='flex flex-col gap-y-2 md:gap-y-4'>
        {piles.map((coins, index) => (
          ...
          <div className='flex items-center' key={index}>
            <div className='text-xs w-5 p-[0.125rem] md:text-sm md:w-7 md:p-1 font-semibold font-mono rounded-full bg-zinc-800 text-white text-center'>{index + 1}</div>&emsp;
            <Pile coins={coins}/>
          </div> : null
        ))}
      </div>
